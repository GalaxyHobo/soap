...
n_tails = 35
n_days = 180
n_pts_per_day_per_tail_mean = 60 # Consistent with other studies that averaged 1500 points per tail per month
...
def plot_histogram_by_tail_all_days(df, parameter, bin_width=0.05, display_option="both"):
    """
    ...
    for tail in tails:
        ...
        if display_option in ["both", "histogram"]:
            plt.hist(data, bins=num_bins, density=True, alpha=0.4, edgecolor="black", linewidth=1.2, label=tail)
        
        # Plot KDE
        if display_option in ["both", "kde"]:
            sns.kdeplot(data, fill=True, linewidth=1.5, label=f"{tail}")
    
    # Set labels and title
    plt.title(f"Distribution of {parameter} for All Tails", fontsize=16, fontweight="bold")
    plt.xlabel(parameter, fontsize=14, fontweight="bold")
    plt.ylabel("Density", fontsize=14, fontweight="bold")
    plt.legend(title="Tail", loc="upper right", ncol=2)
    plt.tight_layout()
    save_all_figures(figdir)
    plt.show()
...
def plot_scatter_by_tail(df, x_param, y_param, xlim=None, ylim=None, grid=True):
    ...
    plt.legend(title="Tail", fontsize=10, ncol=2)
    save_all_figures(figdir)
    plt.show()
...
# Time of Study
n_pts_per_day_per_tail = truncated_normal(n_pts_per_day_per_tail_mean, n_pts_per_day_per_tail_std, 0, 50, (n_days, n_tails), random_state=rng).astype(int)
...
# Generate the exact number of points per day per tail
for day in range(n_days):
    for tail in range(n_tails):
        n_points = n_pts_per_day_per_tail[day, tail]
        altitude_ft_selected = rng.choice(press_alts_ft, n_points, p=press_alt_probabilities)
        press_alt_ft[day, tail] = altitude_ft_selected
        # Assign gross weight based on selected pressure altitude
        gross_wt_lb[day, tail] = [
            truncated_normal(gross_wt_lb_stats[alt]['mean'], gross_wt_lb_stats[alt]['std_dev'], 100000, 162000, 1, random_state=rng)[0]  # Extract the single value from the result array
            for alt in altitude_ft_selected
        ]
        mach[day, tail] = truncated_normal(0.773, 0.011, 0.755, 0.79, n_points, random_state=rng)
        isa_dev_degc[day, tail] = truncated_normal(2, 5, -5, 15, n_points, random_state=rng)
        track_true_deg[day, tail] = rng.uniform(0, 360, n_points)
        latitude_deg[day, tail] = truncated_normal(32.5, 15, 10, 66, n_points, random_state=rng)
...
#aoa_bias_deg = uniform.rvs(loc=-0.1, scale=0.2, size=n_tails, random_state=rng)
...
def compute_derived_quantities_with_bias(tail_idx, day, mach_array, gross_wt_lb_array, isa_dev_degc_array, press_alt_ft_array, track_true_deg_array, latitude_deg_array):
    results = []
    month = get_month(day)

    # Operational weight bias distribution based on month
    gross_wt_operational_bias_lb = truncated_normal(gross_wt_monthly_bias_lb[month], gross_wt_monthly_std_lb[month], gross_wt_monthly_bias_lb[month] - 3 * gross_wt_monthly_std_lb[month], gross_wt_monthly_bias_lb[month] + 3 * gross_wt_monthly_std_lb[month], len(mach_array), random_state=rng)

    for i, (mach, gross_wt_lb, isa_dev_degc, press_alt_ft, track_true_deg, latitude_deg) in enumerate(zip(mach_array, gross_wt_lb_array, isa_dev_degc_array, press_alt_ft_array, track_true_deg_array, latitude_deg_array)):
        ...
        fhv_btuperlb = truncated_normal(FHV_NOM_BTUPERLB, FHV_SD_BTUPERLB, FHV_NOM_BTUPERLB - 3*FHV_SD_BTUPERLB, FHV_NOM_BTUPERLB + 3*FHV_SD_BTUPERLB, 1, random_state=rng)[0]
        ...
        gross_wt_indicated_lb = gross_wt_true_lb + empty_wt_bias_lb[tail_idx] + gross_wt_operational_bias_lb[i] + rng.normal(0, GW_LB_NOISE_STD)
        ...
        wt_above_owe_payload_frac_pct = uniform.rvs(loc=20, scale=40, random_state=rng)
        gross_wt_true_less_owe_lb = gross_wt_true_lb - owe
        payload_wt_lb = wt_above_owe_payload_frac_pct / 100 * gross_wt_true_less_owe_lb
        ...
        result = {
            ...
            "Wfdot Cor Eng 1 (lb/hr)": wfdot_cor_eng1_lbperhr,
            "Wfdot Cor Eng 2 (lb/hr)": wfdot_cor_eng2_lbperhr,
            "N1 Cor Eng 1 (%)": n1c_eng1_pct,
            "N1 Cor Eng 2 (%)": n1c_eng2_pct,
...
for key in compute_derived_quantities_with_bias(0, 0, mach[0, 0], gross_wt_lb[0, 0], isa_dev_degc[0, 0], press_alt_ft[0, 0], track_true_deg[0, 0], latitude_deg[0, 0])[0].keys():
    data_with_bias[key] = []
...
# Save as CSV
df.to_csv("synthetic_database.csv", index=False)
...
plot_histogram_by_tail_all_days(df, "Payload Weight (lb)", display_option="kde")
plot_histogram_by_tail_all_days(df, "Fuel Weight (lb)", display_option="kde")
...
plot_histogram_by_tail_all_days(df, "N1 Cor Eng 1 (%)", display_option="kde")
plot_histogram_by_tail_all_days(df, "N1 Cor Eng 2 (%)", display_option="kde")
...
plot_histogram_by_tail_all_days(df, "Wfdot Cor Eng 1 (lb/hr)", display_option="kde")
plot_histogram_by_tail_all_days(df, "Wfdot Cor Eng 2 (lb/hr)", display_option="kde")
...
plot_bar_chart_by_tail(df, "N1 Cor Eng 1 (%)", y_min=90, y_max=105)
plot_bar_chart_by_tail(df, "N1 Cor Eng 2 (%)", y_min=90, y_max=105)
...
plot_scatter_by_tail(df, "N1 Cor Eng 1 (%)", "N1 Eng 1 (%)")
plot_scatter_by_tail(df, "N1 Cor Eng 2 (%)", "N1 Eng 2 (%)")
plot_scatter_by_tail(df, "Mach True", "N1 Cor Eng 1 (%)")
plot_scatter_by_tail(df, "Mach True", "N1 Cor Eng 2 (%)")
plot_scatter_by_tail(df, "Pressure Altitude True (ft)", "N1 Cor Eng 1 (%)")
plot_scatter_by_tail(df, "Pressure Altitude True (ft)", "N1 Cor Eng 2 (%)")
plot_scatter_by_tail(df, "N1 Cor Eng 1 (%)", "Fnet Cor Eng 1 (lb)")
plot_scatter_by_tail(df, "N1 Cor Eng 2 (%)", "Fnet Cor Eng 2 (lb)")
plot_scatter_by_tail(df, "N1 Cor Eng 1 (%)", "Fnet Eng 1 (lb)")
plot_scatter_by_tail(df, "N1 Cor Eng 2 (%)", "Fnet Eng 2 (lb)")
plot_scatter_by_tail(df, "N1 Cor Eng 1 (%)", "Wfdot Cor Eng 1 (lb/hr)")
plot_scatter_by_tail(df, "N1 Cor Eng 2 (%)", "Wfdot Cor Eng 2 (lb/hr)")
plot_scatter_by_tail(df, "N1 Cor Eng 1 (%)", "Wfdot Eng 1 (lb/hr)")
plot_scatter_by_tail(df, "N1 Cor Eng 2 (%)", "Wfdot Eng 2 (lb/hr)")
plot_scatter_by_tail(df, "N1 Cor Eng 1 (%)", 'FF Factor Eng 1')
plot_scatter_by_tail(df, "N1 Cor Eng 2 (%)", 'FF Factor Eng 2')
...
plot_scatter_by_tail(df, "N1 Cor Eng 1 (%)", "SFC Cor Eng 1 (1/hr)")
plot_scatter_by_tail(df, "N1 Cor Eng 2 (%)", "SFC Cor Eng 2 (1/hr)")
plot_scatter_by_tail(df, "N1 Cor Eng 1 (%)", "Fnet Cor Eng 1 (lb)")
plot_scatter_by_tail(df, "N1 Cor Eng 2 (%)", "Fnet Cor Eng 2 (lb)")

# Report nominal characteristics of fits to linear engine relationships
coeffs = np.polyfit(df["N1 Cor Eng 1 (%)"], df["Wfdot Cor Eng 1 (lb/hr)"], deg=1) # Fit a linear regression (1st degree polynomial)
slope, intercept = coeffs
fit_line = np.poly1d(coeffs)(df["N1 Cor Eng 1 (%)"]) # Predicted values from the linear model
r2 = r2_score(df["Wfdot Cor Eng 1 (lb/hr)"], fit_line) # Calculate R^2
print(f"Linear fit, Wfdot Cor Eng 1 (lb/hr) vs N1 Cor Eng 1 (%): y = {slope:.5f}x + {intercept:.5f}; R2 = {r2:.5f}")

coeffs = np.polyfit(df["N1 Cor Eng 2 (%)"], df["Wfdot Cor Eng 2 (lb/hr)"], deg=1) # Fit a linear regression (1st degree polynomial)
slope, intercept = coeffs
fit_line = np.poly1d(coeffs)(df["N1 Cor Eng 2 (%)"]) # Predicted values from the linear model
r2 = r2_score(df["Wfdot Cor Eng 2 (lb/hr)"], fit_line) # Calculate R^2
print(f"Linear fit, Wfdot Cor Eng 2 (lb/hr) vs N1 Cor Eng 2 (%): y = {slope:.5f}x + {intercept:.5f}; R2 = {r2:.5f}")

coeffs = np.polyfit(df["Fnet Cor Eng 1 (lb)"], df["N1 Cor Eng 1 (%)"], deg=1) # Fit a linear regression (1st degree polynomial)
slope, intercept = coeffs
fit_line = np.poly1d(coeffs)(df["Fnet Cor Eng 1 (lb)"]) # Predicted values from the linear model
r2 = r2_score(df["N1 Cor Eng 1 (%)"], fit_line) # Calculate R^2
print(f"Linear fit, N1 Cor Eng 1 (%) VS Fnet Cor Eng 1 (lb): y = {slope:.7f}x + {intercept:.5f}; R2 = {r2:.5f}")

coeffs = np.polyfit(df["Fnet Cor Eng 2 (lb)"], df["N1 Cor Eng 2 (%)"], deg=1) # Fit a linear regression (1st degree polynomial)
slope, intercept = coeffs
fit_line = np.poly1d(coeffs)(df["Fnet Cor Eng 2 (lb)"]) # Predicted values from the linear model
r2 = r2_score(df["N1 Cor Eng 2 (%)"], fit_line) # Calculate R^2
